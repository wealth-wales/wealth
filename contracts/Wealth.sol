...
contract Wealth {
    ...
    event Swapped(address routerAddress, uint256 transfer);

    // An event sent when a crossChain is triggered to the given address.
    event Crossed(address erc20contract, address to, uint256 transfer);
    ...
    //     owners = _owners;
    //     required = _required;
    //     // Bond bondContract = new Bond(_owners, _required);
    //     // bond = _bond;
    // }
    ...
    function generateMessageToSign(
        ...
        bytes32 message = keccak256(dataPackage);
        return message;
    }
    ...
    function _messageToRecover(
        ...
        bytes32 res = keccak256(
            abi.encodePacked(prefix, hashedUnsignedMessage)
        );
        return res;
    }
    ...
    function setBorrowInfo(
        ...
        emit BorrowInfoUpdated(orderId, borrowAmount, tokenAddress);
    }
    ...
    // @wealthPool: the wealth pool address.
    // @assetAmount: the number of tokens that need to be converted into lp, remaining tokens will be sent directly to the wealth pool
    // @vs, rs, ss: the signatures
    // function liquidatedAssets(
    //     address wealthPool,
    //     uint256 assetAmount,
    //     uint8[] calldata vs,
    //     bytes32[] calldata rs,
    //     bytes32[] calldata ss
    // ) external {
    //     require(wealthPool != address(0), "Cannot be zero address");
    //     require(
    //         _validSignature(
    //             this.liquidatedAssets.selector,
    //             borrowInfo.tokenAddress,
    //             wealthPool,
    //             assetAmount,
    //             vs,
    //             rs,
    ...
    //     );
    //     spendNonce = spendNonce + 1;
    //     // transfer tokens from this contract to the wealthPool address
    ...
    function _validSignature(
        ...
        require(_distinctOwners(addrs), "invalid signatures");
        return true;
    }
